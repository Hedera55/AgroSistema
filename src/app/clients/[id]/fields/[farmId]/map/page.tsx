...
export default function FarmMapPage({ params }: { params: Promise<{ id: string, farmId: string }> }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // 1. Fetch current farm
                const farmData = await db.get('farms', farmId);
                if (farmData) {
                    setCurrentFarm(farmData);
                    setSelectedLayerIds(new Set([farmId])); // Select current farm by default
                }
    ...
    if (!currentFarm || !currentFarm.boundary) {
        return (
            <div className="p-8 text-center">
                <h1 className="text-2xl font-bold text-slate-900 mb-4">Mapa no disponible</h1>
                <p className="text-slate-500 mb-6">No se ha cargado un KML para este campo.</p>
        ...
        <div className="flex flex-col h-[calc(100vh-120px)] space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{currentFarm.name} - Mapa Campo</h1>
                </div>
            </div>
