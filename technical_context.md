...
#### 🚜 Harvest Batch & Movement Policy
- **Field Wizard**: All movements generated by the `HarvestWizard` (Entries & Direct Sales) MUST include the `harvestBatchId`.
  - **Reason**: Allows the wizard to find and adjust these specific linked movements during edits.
- **Stock Page (Galpón)**: Movements generated from the Stock Page (Withdrawals/Sales) MUST NOT include a `harvestBatchId`.
  - **Reason**: These are aggregation-level consumption of the "Global Product Row."
- **Broad Sense Warning**: Deleting a harvest is non-blocking but triggers a **Broad Sense Warning** if the Product (by Fingerprint/ID) has **any** history of sales or withdrawals, regardless of direct batch links.
  - **Message**: *"Aviso: Este producto ya tiene ventas o retiros registrados en el historial (Galpón o Directos). Al borrar esta cosecha, el stock podría quedar en negativo hasta que lo corrija."*
- **Manual Integrity**: The ledger (Stock Table) flags negative balances in Red with an audit tooltip to guide manual correction, prioritizing transparency over rigid system blocks.
- **Audit Tooltip**: Any stock row with a negative balance MUST display an alarm with the text: *"Revise el historial, hay más producto usado que comprado/cosechado."*
