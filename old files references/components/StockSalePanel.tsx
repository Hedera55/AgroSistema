        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xl relative animate-fadeIn mb-4 floating-panel" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-6">
                <h2 className="text-lg font-semibold text-slate-800">
                    {titleOverride || 'Venta'} ({isGrainOrSeed 
                        ? `${stockItem.productName} ${(!stockItem.productCommercialName || stockItem.productCommercialName === 'Común') ? 'Propia' : stockItem.productCommercialName}` 
                        : (stockItem.productBrand === 'Común' ? '---' : (stockItem.productBrand || '---'))})
                </h2>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">
                    Disponible: {isGrainOrSeed ? (stockItem.quantity / 1000).toLocaleString(undefined, { maximumFractionDigits: 3 }) : stockItem.quantity} {isGrainOrSeed ? 'Tons' : stockItem.unit}
                </p>
            </div>
            ...
            <form onSubmit={onSubmit} className="flex flex-col gap-6">
                ...
                <div className="flex flex-wrap items-end gap-6">
                    <div className="flex-1 min-w-[200px]">
                        <Input
                            label={`Cantidad a Vender (${isGrainOrSeed ? 'Tons' : (stockItem.unit || 'uds')})`}
                            type="text"
                            inputMode="decimal"
                            value={saleQuantity}
                    ...
                    <div className="flex-1 min-w-[200px]">
                        <Input
                            label={`Precio de Venta (USD/${isGrainOrSeed ? 'Ton' : (stockItem.unit || 'ud')})`}
                            type="text"
                            inputMode="decimal"
                            value={salePrice}
